A modernized, cross-platform CMake wrapper for the MIT NNLS package (optml.mit.edu/work/soft/nnls.html).
This project adds structured CMake build machinery to ensure consistent compilation,
dependency management, and installation across Linux, macOS, and Windows.
...
1. project is configured via cmake-gui
2. create a key BLAS_LIBRARIES to point to existing BLAS library.
   Or you can set the keys following the instruction in cmake's FindBLAS module

3. some other useful keys
   - CMAKE_INSTALL_PREFIX: installation destination
   - BUILD_SHARED_LIBS: build the shared library (OFF by default)
...
The distribution includes the library and a driver program, nnlsDriver.cc
(which is the file that is compiled by make to yield 'nnls'); this driver
illustrates how to use libnnls to solve an nnls problem.
This driver is very rudimentary, but serves the purpose.
...
1. Add to the cmake configuration

  - CMAKE_PREFIX_PATH="%NNLS_ROOT%\share\cmake\nnls"

2. Linking to the project is straightforward with the cmake command

  - target_link_libraries(your_program PUBLIC SS::nnls)
